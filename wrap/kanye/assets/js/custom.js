...
$(document).ready(function($) {
    ...
    // CDN paths for third‑party libraries
    var cdnPaths = {
        ...
        isotope: "https://cdnjs.cloudflare.com/ajax/libs/jquery.isotope/3.0.6/isotope.pkgd.min.js",
        particles: "https://cdnjs.cloudflare.com/ajax/libs/particles.js/2.0.0/particles.min.js",
        typeit: "https://cdn.jsdelivr.net/npm/typeit@8.7.1/dist/typeit.min.js",
        countTo: "https://cdn.jsdelivr.net/npm/jquery-countto@1.2.0/jquery.countTo.min.js"
    };
    ...
    * OwlSlider
    ...
    WEA.Owl = function() {
        var owlslider = $("div.owl-carousel");
        if (owlslider.length > 0) {
            loadScript(cdnPaths.owlCarousel, function() {
                owlslider.each(function() {
                    var $this = $(this),
                        $items = ($this.data('items')) ? $this.data('items') : 1,
                        $loop = ($this.attr('data-loop')) ? $this.data('loop') : true,
    ...
    WEA.Gallery = function() {
        if ($(".lightbox-gallery").exists() || $(".popup-youtube, .popup-vimeo, .popup-gmaps").exists()) {
            loadScript(cdnPaths.magnificPopup, function() {
                if ($(".lightbox-gallery").exists()) {
                    $('.lightbox-gallery').magnificPopup({
                        delegate: '.gallery-link',
                        type: 'image',
    ...
    * Masonry
    ...
    WEA.masonry = function() {
        var portfolioWork = $('.portfolio-content');
        if ($(".portfolio-content").exists()) {
            loadScript(cdnPaths.isotope, function() {
                if ($(".portfolio-content").exists()) {
                    $(portfolioWork).isotope({
                        resizable: false,
                        itemSelector: '.grid-item',
    ...
    WEA.particles = function() {
        if ($("#particles-box").exists()) {
            loadScript(cdnPaths.particles, function() {
                // If you have custom particle configuration, load it separately.
                loadScript("wrap/kanye/assets/vendor/particles/particles-app.js", function() {});
            });
        }
    };
    ...
    WEA.mTypeIt = function() {
        if ($("#type-it").exists()) {
            loadScript(cdnPaths.typeit, function() {
                new TypeIt('#type-it', {
                    speed: 200,
                    loop: true,
                    strings: [
    ...
    * Counter JS
    ...
    WEA.Counter = function() {
        var $counter = $('.counter');
        if ($counter.length > 0) {
            loadScript(cdnPaths.countTo, function() {
                $counter.each(function() {
                    var $elem = $(this);
                    $elem.appear(function() {
                        $elem.find('.count').countTo({
    ...
    /* ---------------------------------------------- /*
     * loadScript function using caching to avoid duplicate loads
    /* ---------------------------------------------- */
    var _arr = {};
